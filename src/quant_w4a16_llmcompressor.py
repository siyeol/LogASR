"""GPTQ W4A16 of Whisper-large-v3 via llm-compressor.

Calibration uses local LibriSpeech test-other so test-clean eval is held out.
"""
...
SAVE_DIR = "/workspace/SpeechPTQ/models/whisper-large-v3-W4A16-G128"
...
def main():
    ...
    recipe = GPTQModifier(
        targets="Linear",
        scheme="W4A16",
        ignore=["lm_head", "re:.*proj_out"],
        dampening_frac=0.01,
    )

    print("[gptq] running oneshot W4A16")
    oneshot(
        model=model,
        dataset=ds,
        recipe=recipe,
